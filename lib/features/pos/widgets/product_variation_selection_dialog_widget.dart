          ...
          child: Consumer<ProductController>(
            builder: (ctx, productProvider, child) {
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                CustomButtonWidget(btnTxt: getTranslated(stock! < widget.product!.minimumOrderQty! && widget.product!.productType == "physical"?
                'out_of_stock' : 'add_to_cart', context),
                    onTap: stock < widget.product!.minimumOrderQty!  &&  widget.product!.productType == "physical" ? null :() {
                      ...
                      if(stock! >= widget.product!.minimumOrderQty!  || widget.product!.productType == "digital") {
                        CartModel cart = CartModel( widget.product!.unitPrice, widget.product!.discount, productProvider.quantity??1, widget.product!.tax, variationModel?.type ?? null, variationModel, variantKey, digitalVariantPrice, widget.product, widget.product!.taxModel);
                        print("====>>ELSECartModel=====>>${cart.toJson()}");
                        Provider.of<CartController>(context, listen: false).addToCart(context, cart);
                        Navigator.pop(context);
                      }
                }),
                const SizedBox(width: Dimensions.paddingSizeDefault),
